...
%         b               = double(string(phase));
        c               = find(phase > 7,1); %Want to remove this truncation and instead truncate sessions at correct date or max number of training sessions
        phase(c:end)    = [];
...
%         correct         = renamecats(corr,["true" "false"],["correct" "incorrect"]); %I don't think this logical was used for anything, but I was maybe planning to use it for counting reward delivery
        A               = [phase,side1,shape1,color1]; %can add back "correct" if I comment out the above lines - this for winshift and winstay
        outpt{i}        = cellstr(A);
